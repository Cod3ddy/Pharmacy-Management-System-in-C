void registration(){
	setWindowState("Pharmacy (Registration)");
	system("cls");
	char password1 [20];
	char password2 [20];
	FILE *fp;
	fp = fopen("files/customer/ctmCredentials.txt", "ab+");
	if(fp == NULL){
 ...
 void login(){
	setWindowState("Pharmacy (Sign in)");
	system("cls");
	int status = 0;
	FILE *fp;
	pos(22, 4);
	...
	fp = fopen("files/customer/ctmCredentials.txt", "rb+");
	if(fp == NULL){
		printf ("sorry, file not found");
		exit(0);
	}else{
